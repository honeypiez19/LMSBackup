...
LEFT JOIN employees leader ON (
    leader.e_level IN ('manager','assisManager')
    AND (
...
WHERE 
    ...
    AND li.l_level IN ('user','leader','chief','admin')";
...
foreach ($leaveRecords as $row) {
    ...
    if ($workplace === 'Bang Phli' && $daysDifference == 2) {
        ...
        if ($depart == 'RD') {
            $sqlLine = "SELECT e_token, e_username FROM employees WHERE e_workplace = :workplace AND e_level = 'manager' AND e_sub_department = 'RD'";
            ...
            if($subDepart == 'Modeling'){
                $sqlLine = "SELECT e_token, e_username FROM employees WHERE e_level = 'assisManager'";
            }
